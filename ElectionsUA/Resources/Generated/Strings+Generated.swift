// swiftlint:disable all
// Generated using SwiftGen — https://github.com/SwiftGen/SwiftGen

import Foundation

// swiftlint:disable superfluous_disable_command file_length implicit_return prefer_self_in_static_references

// MARK: - Strings

// swiftlint:disable explicit_type_interface function_parameter_count identifier_name line_length
// swiftlint:disable nesting type_body_length type_name vertical_whitespace_opening_braces
public enum Strings {
  public enum LawsTab {
    public enum About {
      /// одна з провідних неурядових та позапартійних всеукраїнських організацій громадського контролю у сфері виборів. З 2007 року ОПОРА аналізує виборчий процес, проводить спостереження за всіма його етапами, працює над удосконаленням виборчого законодавства відповідно до міжнародних стандартів. Загалом за цей час до моніторингу виборів було залучено понад 20 тисяч кваліфікованих спостерігачів. 
      /// 
      ///  Для своїх спостерігачів, та усіх інших учасників виборчого процесу ми розробили цей мобільний додаток із виборчим кодексом та верифікатором протоколу, який знадобиться вам у ніч підрахунку голосів. Тепер виборчий кодекс буде завжди із вами, а зручний пошук дозволить швидко знайти необхідну статтю. Отримати усю оперативну інформацію про перебіг виборів в Україні Ви можете тут: 
      /// 
      public static let body = Strings.tr("LawsTab", "about.Body", fallback: "одна з провідних неурядових та позапартійних всеукраїнських організацій громадського контролю у сфері виборів. З 2007 року ОПОРА аналізує виборчий процес, проводить спостереження за всіма його етапами, працює над удосконаленням виборчого законодавства відповідно до міжнародних стандартів. Загалом за цей час до моніторингу виборів було залучено понад 20 тисяч кваліфікованих спостерігачів. \n\n Для своїх спостерігачів, та усіх інших учасників виборчого процесу ми розробили цей мобільний додаток із виборчим кодексом та верифікатором протоколу, який знадобиться вам у ніч підрахунку голосів. Тепер виборчий кодекс буде завжди із вами, а зручний пошук дозволить швидко знайти необхідну статтю. Отримати усю оперативну інформацію про перебіг виборів в Україні Ви можете тут: \n")
      /// І не забувайте стежити за нами в соціальних мережах:
      public static let end = Strings.tr("LawsTab", "about.End", fallback: "І не забувайте стежити за нами в соціальних мережах:")
      /// Facebook
      public static let facebook = Strings.tr("LawsTab", "about.facebook", fallback: "Facebook")
      /// Instagram
      public static let instagram = Strings.tr("LawsTab", "about.instagram", fallback: "Instagram")
      /// Telegram
      public static let telegram = Strings.tr("LawsTab", "about.telegram", fallback: "Telegram")
      /// Громадянська мережа ОПОРА – 
      /// 
      /// 
      public static let title = Strings.tr("LawsTab", "about.Title", fallback: "Громадянська мережа ОПОРА – \n\n")
      /// Twitter
      public static let twitter = Strings.tr("LawsTab", "about.twitter", fallback: "Twitter")
      /// Website
      public static let website = Strings.tr("LawsTab", "about.website", fallback: "Website")
      /// YouTube
      public static let youtube = Strings.tr("LawsTab", "about.youtube", fallback: "YouTube")
    }
    public enum Law {
      /// Кодекс України
      public static let codeOfUkraine = Strings.tr("LawsTab", "law.CodeOfUkraine", fallback: "Кодекс України")
      /// Виборчий
      public static let electoral = Strings.tr("LawsTab", "law.Electoral", fallback: "Виборчий")
      /// Закон України
      public static let lawOfUkraine = Strings.tr("LawsTab", "law.LawOfUkraine", fallback: "Закон України")
      /// Про вибори народних депутатів України
      public static let onDeputies = Strings.tr("LawsTab", "law.OnDeputies", fallback: "Про вибори народних депутатів України")
      /// Про політичні партії
      public static let onParties = Strings.tr("LawsTab", "law.OnParties", fallback: "Про політичні партії")
      /// Про всеукраїнський референдум
      public static let onReferendum = Strings.tr("LawsTab", "law.OnReferendum", fallback: "Про всеукраїнський референдум")
    }
    public enum Link {
      /// https://www.facebook.com/cn.opora
      public static let facebook = Strings.tr("LawsTab", "link.facebook", fallback: "https://www.facebook.com/cn.opora")
      /// https://www.instagram.com/cn.opora/
      public static let instagram = Strings.tr("LawsTab", "link.instagram", fallback: "https://www.instagram.com/cn.opora/")
      /// https://t.me/opora_news
      public static let telegram = Strings.tr("LawsTab", "link.telegram", fallback: "https://t.me/opora_news")
      /// https://twitter.com/opora
      public static let twitter = Strings.tr("LawsTab", "link.twitter", fallback: "https://twitter.com/opora")
      /// https://oporaua.org/
      public static let website = Strings.tr("LawsTab", "link.website", fallback: "https://oporaua.org/")
      /// https://www.youtube.com/user/oporavideo
      public static let youTube = Strings.tr("LawsTab", "link.youTube", fallback: "https://www.youtube.com/user/oporavideo")
    }
    public enum LinkText {
      /// Аналізатор політичної реклами 
      ///  
      /// 
      public static let analizatorString = Strings.tr("LawsTab", "linkText.AnalizatorString", fallback: "Аналізатор політичної реклами \n \n")
      /// Все про виборчі дані та соцмережі 
      /// 
      public static let daniString = Strings.tr("LawsTab", "linkText.DaniString", fallback: "Все про виборчі дані та соцмережі \n")
      /// Мапа порушень 
      /// 
      public static let mapString = Strings.tr("LawsTab", "linkText.MapString", fallback: "Мапа порушень \n")
      /// Вебсайт 
      /// 
      public static let websiteString = Strings.tr("LawsTab", "linkText.WebsiteString", fallback: "Вебсайт \n")
    }
    public enum NavTitle {
      /// Кодекс
      public static let code = Strings.tr("LawsTab", "navTitle.Code", fallback: "Кодекс")
      /// Депутати
      public static let deputies = Strings.tr("LawsTab", "navTitle.Deputies", fallback: "Депутати")
      /// Партії
      public static let parties = Strings.tr("LawsTab", "navTitle.Parties", fallback: "Партії")
      /// Референдум
      public static let referendum = Strings.tr("LawsTab", "navTitle.Referendum", fallback: "Референдум")
    }
    public enum Placeholder {
      /// ЩОСЬ ЗНАЙТИ?
      public static let findSomething = Strings.tr("LawsTab", "placeholder.FindSomething", fallback: "ЩОСЬ ЗНАЙТИ?")
      /// НІЧОГО НЕ ЗНАЙДЕНО
      public static let nothingFound = Strings.tr("LawsTab", "placeholder.NothingFound", fallback: "НІЧОГО НЕ ЗНАЙДЕНО")
    }
    public enum Realm {
      /// ElectoralCode.realm
      public static let electoralCode = Strings.tr("LawsTab", "realm.ElectoralCode", fallback: "ElectoralCode.realm")
      /// ElectoralCode
      public static let electoralCodeTitle = Strings.tr("LawsTab", "realm.ElectoralCodeTitle", fallback: "ElectoralCode")
      /// NardepyLaw.realm
      public static let nardepyLaw = Strings.tr("LawsTab", "realm.NardepyLaw", fallback: "NardepyLaw.realm")
      /// NardepyLaw
      public static let nardepyLawTitle = Strings.tr("LawsTab", "realm.NardepyLawTitle", fallback: "NardepyLaw")
      /// PartiesLaw.realm
      public static let partiesLaw = Strings.tr("LawsTab", "realm.PartiesLaw", fallback: "PartiesLaw.realm")
      /// PartiesLaw
      public static let partiesLawTitle = Strings.tr("LawsTab", "realm.PartiesLawTitle", fallback: "PartiesLaw")
      /// realm
      public static let realm = Strings.tr("LawsTab", "realm.realm", fallback: "realm")
      /// ReferendumLaw.realm
      public static let referendumLaw = Strings.tr("LawsTab", "realm.ReferendumLaw", fallback: "ReferendumLaw.realm")
      /// ReferendumLaw
      public static let referendumLawTitle = Strings.tr("LawsTab", "realm.ReferendumLawTitle", fallback: "ReferendumLaw")
    }
    public enum Tab {
      /// Обране
      public static let favourite = Strings.tr("LawsTab", "tab.Favourite", fallback: "Обране")
      /// LawsTab.strings
      ///   ElectionsUA
      /// 
      ///   Created by Oksana Fedorchuk on 21.04.2022.
      public static let laws = Strings.tr("LawsTab", "tab.Laws", fallback: "Закони")
      /// Протокол
      public static let `protocol` = Strings.tr("LawsTab", "tab.Protocol", fallback: "Протокол")
      /// Пошук
      public static let search = Strings.tr("LawsTab", "tab.Search", fallback: "Пошук")
    }
  }
  public enum ProtocolTab {
    public enum `Protocol` {
      /// Кандидат 1
      public static let candidate1 = Strings.tr("ProtocolTab", "protocol.Candidate1", fallback: "Кандидат 1")
      /// Кандидат 2
      public static let candidate2 = Strings.tr("ProtocolTab", "protocol.Candidate2", fallback: "Кандидат 2")
      /// Кількість одержаних бюлетенів (п1) не дорівнює сумі кількості невикористаних бюлетенів (п2) Загальній кількості виборців
      public static let error1n2 = Strings.tr("ProtocolTab", "protocol.error1n2", fallback: "Кількість одержаних бюлетенів (п1) не дорівнює сумі кількості невикористаних бюлетенів (п2) Загальній кількості виборців")
      /// Кількість виборців, включених до списку виборців на виборчій дільниці (п.3), менша за загальну кількість виборців, які отримали виборчі бюлетені (п.7)
      public static let error3n7 = Strings.tr("ProtocolTab", "protocol.error3n7", fallback: "Кількість виборців, включених до списку виборців на виборчій дільниці (п.3), менша за загальну кількість виборців, які отримали виборчі бюлетені (п.7)")
      /// Кількість виборців, внесених до витягу із списку виборців для голосування за місцем перебування (п.4), менша за кількість виборців, які отримали виборчі бюлетені за місцем перебування (п.6)
      public static let error4n6 = Strings.tr("ProtocolTab", "protocol.error4n6", fallback: "Кількість виборців, внесених до витягу із списку виборців для голосування за місцем перебування (п.4), менша за кількість виборців, які отримали виборчі бюлетені за місцем перебування (п.6)")
      /// Кількість виборців, які отримали виборчі бюлетені (п7) не дорівнює сумі кількостей виборців, які отримали виборчі бюлетені у приміщені для голосування (п5) та за місцем перебування (п6)
      public static let error7n5n6 = Strings.tr("ProtocolTab", "protocol.error7n5n6", fallback: "Кількість виборців, які отримали виборчі бюлетені (п7) не дорівнює сумі кількостей виборців, які отримали виборчі бюлетені у приміщені для голосування (п5) та за місцем перебування (п6)")
      /// Кількість виборців які взяли участь у голосуванні(п9) не дорівнює сумі голосів за кандидатів(п 11) та кількості недійсних бюлетенів (п 10)
      public static let error9n11n10 = Strings.tr("ProtocolTab", "protocol.error9n11n10", fallback: "Кількість виборців які взяли участь у голосуванні(п9) не дорівнює сумі голосів за кандидатів(п 11) та кількості недійсних бюлетенів (п 10)")
      /// Ти впевнений, що ці бюлетені не підлягають врахуванню? Нагадаємо, не підлягають врахуванню лише наступні бюлетені:
      ///         - усі бюлетені у виборчій скринці у разі відсутності контрольоного листа;
      ///         - усі бюлетені у виборчій скринці у разі виникнення сумнівів щодо достовірності контрольного листа
      ///         -усі бюлетені у переносній виборчій скринці у разі наявності в ній більшої кількості бюлетенів, ніж виборців включених до витягну зі списку виборців
      ///         -усі бюлетені на яких зазначені номери виборчого округу та дільниці не відповідають номеру відповідного округу;
      public static let errorHint = Strings.tr("ProtocolTab", "protocol.errorHint", fallback: "Ти впевнений, що ці бюлетені не підлягають врахуванню? Нагадаємо, не підлягають врахуванню лише наступні бюлетені:\n        - усі бюлетені у виборчій скринці у разі відсутності контрольоного листа;\n        - усі бюлетені у виборчій скринці у разі виникнення сумнівів щодо достовірності контрольного листа\n        -усі бюлетені у переносній виборчій скринці у разі наявності в ній більшої кількості бюлетенів, ніж виборців включених до витягну зі списку виборців\n        -усі бюлетені на яких зазначені номери виборчого округу та дільниці не відповідають номеру відповідного округу;")
      /// Кількість виборчих бюлетенів, що не підлягають врахуванню
      public static let totalBallotsNotIncluded = Strings.tr("ProtocolTab", "protocol.TotalBallotsNotIncluded", fallback: "Кількість виборчих бюлетенів, що не підлягають врахуванню")
      /// ProtocolTab.strings
      ///   ElectionsUA
      /// 
      ///   Created by Oksana Fedorchuk on 30.05.2022.
      public static let totalBallotsReceived = Strings.tr("ProtocolTab", "protocol.TotalBallotsReceived", fallback: "Кількість виборчих бюлетенів, одержаних дільничною виборчою комісією")
      /// Кількість невикористаних виборчих бюлетенів, погашених дільничною виборчою комісією
      public static let totalBallotsUnused = Strings.tr("ProtocolTab", "protocol.TotalBallotsUnused", fallback: "Кількість невикористаних виборчих бюлетенів, погашених дільничною виборчою комісією")
      /// Кількість виборчих бюлетенів, визнаних недійсними
      public static let totalBallotsUnvalid = Strings.tr("ProtocolTab", "protocol.TotalBallotsUnvalid", fallback: "Кількість виборчих бюлетенів, визнаних недійсними")
      /// Кількість виборців, внесених на виборчій дільниці до витягу із списку виборців для голосування за місцем перебування
      public static let totalVotersAtHome = Strings.tr("ProtocolTab", "protocol.TotalVotersAtHome", fallback: "Кількість виборців, внесених на виборчій дільниці до витягу із списку виборців для голосування за місцем перебування")
      /// Кількість виборців, включених до списку виборців на виборчій дільниці
      public static let totalVotersEnlisted = Strings.tr("ProtocolTab", "protocol.TotalVotersEnlisted", fallback: "Кількість виборців, включених до списку виборців на виборчій дільниці")
      /// Загальна кількість виборців, які отримали виборчі бюлетені
      public static let totalVotersReceivedBallots = Strings.tr("ProtocolTab", "protocol.TotalVotersReceivedBallots", fallback: "Загальна кількість виборців, які отримали виборчі бюлетені")
      /// Кількість виборців, які отримали виборчі бюлетені за місцем перебування
      public static let totalVotersReceivedBallotsAtHome = Strings.tr("ProtocolTab", "protocol.TotalVotersReceivedBallotsAtHome", fallback: "Кількість виборців, які отримали виборчі бюлетені за місцем перебування")
      /// Кількість виборців, які отримали виборчі бюлетені у приміщенні для голосування
      public static let totalVotersReceivedBallotsAtPrecinct = Strings.tr("ProtocolTab", "protocol.TotalVotersReceivedBallotsAtPrecinct", fallback: "Кількість виборців, які отримали виборчі бюлетені у приміщенні для голосування")
      /// Кількість виборців, які взяли участь у голосуванні на виборчій дільниці
      public static let totalVotersVotedAtPrecincts = Strings.tr("ProtocolTab", "protocol.TotalVotersVotedAtPrecincts", fallback: "Кількість виборців, які взяли участь у голосуванні на виборчій дільниці")
      /// Верифікатор протоколів ДВК
      /// Забудь про уточнені протоколи!
      /// Перевіряй протоколи ДВК на анявність логічних помилок за допомогою нашого верифікатора. Усе що треба: обрати відповідний тип протоколи та ввести його числові дані. Ми ж у свою чергу, все перевіримо та порахуємо, а у разі виявлення помилок - обов'язково тебе повідомимо, та вкажемо на усі неточності!
      public static let welcome = Strings.tr("ProtocolTab", "protocol.Welcome", fallback: "Верифікатор протоколів ДВК\nЗабудь про уточнені протоколи!\nПеревіряй протоколи ДВК на анявність логічних помилок за допомогою нашого верифікатора. Усе що треба: обрати відповідний тип протоколи та ввести його числові дані. Ми ж у свою чергу, все перевіримо та порахуємо, а у разі виявлення помилок - обов'язково тебе повідомимо, та вкажемо на усі неточності!")
    }
  }
}
// swiftlint:enable explicit_type_interface function_parameter_count identifier_name line_length
// swiftlint:enable nesting type_body_length type_name vertical_whitespace_opening_braces

// MARK: - Implementation Details

extension Strings {
  private static func tr(_ table: String, _ key: String, _ args: CVarArg..., fallback value: String) -> String {
    let format = BundleToken.bundle.localizedString(forKey: key, value: value, table: table)
    return String(format: format, locale: Locale.current, arguments: args)
  }
}

// swiftlint:disable convenience_type
private final class BundleToken {
  static let bundle: Bundle = {
    #if SWIFT_PACKAGE
    return Bundle.module
    #else
    return Bundle(for: BundleToken.self)
    #endif
  }()
}
// swiftlint:enable convenience_type
